...
const localeFor = (language: Language): string => language === "pt" ? "pt-PT" : "en-GB";
...
export function firstText(value: unknown): string | undefined {
  ...
  if (Array.isArray(value)) {
    const items = value.map(firstText).filter((item): item is string => Boolean(item));
...
export function formatBitRate(bitsPerSecond: number, language: Language = "en"): string {
...
export function formatShutterSpeed(seconds: number, language: Language = "en"): string {
  if (!Number.isFinite(seconds) || seconds <= 0) return "—";
  if (seconds >= 1) return `${new Intl.NumberFormat(localeFor(language), { maximumFractionDigits: 2 }).format(seconds)} ${language === "pt" ? "s" : "sec"}`;
...
export function formatCoordinate(value: number, language: Language = "en"): string {
  return new Intl.NumberFormat(localeFor(language), { minimumFractionDigits: 5, maximumFractionDigits: 7 }).format(value);
}

export function compassDirection(degrees: number, language: Language = "en"): string {
...
export function formatAspectRatio(width: number, height: number): string {
  ...
  const common: Array<[number, string]> = [[16 / 9, "16:9"], [4 / 3, "4:3"], [3 / 2, "3:2"], [1, "1:1"], [9 / 16, "9:16"], [3 / 4, "3:4"], [2 / 3, "2:3"]];
  ...
  if (left <= 50 && right <= 50) return `${left}:${right}`;
  return `${ratio.toFixed(2)}:1`;
}

export function formatDateValue(value: unknown, language: Language = "en"): string | undefined {
  ...
  if (typeof value === "number" && Number.isFinite(value)) {
    ...
    if (!Number.isNaN(date.getTime())) return new Intl.DateTimeFormat(localeFor(language), { dateStyle: "long", timeStyle: "medium" }).format(date);
  }
  return undefined;
}

export function serialiseMetadata(value: unknown): unknown {
  ...
  const visit = (current: unknown): unknown => {
    ...
    if (typeof current === "object") {
      ...
      for (const [key, nested] of Object.entries(current)) output[key] = visit(nested);
      return output;
    }
...
export interface FlatMetadataEntry {
  path: string;
  key: string;
  value: unknown;
}

export function flattenMetadata(value: unknown, parent = "", output: FlatMetadataEntry[] = []): FlatMetadataEntry[] {
  if (Array.isArray(value)) {
