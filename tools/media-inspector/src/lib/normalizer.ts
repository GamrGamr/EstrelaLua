...
function trackType(track: MediaInfoTrack): string {
  return firstText(track["@type"])?.toLowerCase() ?? "other";
}

function numberFrom(track: MediaInfoTrack | undefined, keys: string[]): number | undefined {
  return track ? findMetadataNumber(track, keys) : undefined;
}

function textFrom(track: MediaInfoTrack | undefined, keys: string[]): string | undefined {
  return track ? findMetadataText(track, keys) : undefined;
}

function parseIso6709(value: string | undefined): Pick<LocationMetadata, "latitude" | "longitude" | "altitude"> | undefined {
...
function imageLocation(parsed: ImageParseResult): LocationMetadata | undefined {
  ...
  if (latitude === undefined || longitude === undefined || !isValidCoordinate(latitude, longitude)) return undefined;
...
function videoLocation(raw: Record<string, unknown>): LocationMetadata | undefined {
  ...
  if (isoLocation) return isoLocation;
...
function normaliseVideoTrack(track: MediaInfoTrack, index: number): NormalisedVideoTrack {
  return {
...
function normaliseAudioTrack(track: MediaInfoTrack, index: number): NormalisedAudioTrack {
  return {
    id: textFrom(track, ["ID", "StreamOrder"]) ?? String(index + 1),
    ...(textFrom(track, ["Format"]) ? { format: textFrom(track, ["Format"]) } : {}),
    ...(textFrom(track, ["CodecID", "CodecID_Hint", "InternetMediaType"]) ? { codec: textFrom(track, ["CodecID", "CodecID_Hint", "InternetMediaType"]) } : {}),
...
function deviceFrom(source: unknown): DeviceMetadata {
...
function captureFrom(source: unknown): CaptureMetadata {
...
function datesFrom(source: unknown, file: File): DateMetadata {
...
function fileMetadata(file: File, detected: DetectedMedia, image: ImageParseResult | undefined, video: VideoParseResult | undefined): FileMetadata {
...
export function normaliseImage(file: File, detected: DetectedMedia, parsed: ImageParseResult): MediaMetadata {
  const base = {
    detected,
...
export function normaliseVideo(file: File, detected: DetectedMedia, parsed: VideoParseResult): MediaMetadata {
...
export function parseNumberish(value: unknown): number | undefined {
