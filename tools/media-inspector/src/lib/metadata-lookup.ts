...
export function findMetadataValue(source: unknown, candidates: string[]): unknown {
  ...
  for (const candidate of wanted) {
    ...
    if (exact?.value !== undefined && exact.value !== null && (exact.value instanceof Date || firstText(unwrapValue(exact.value)) !== undefined)) return unwrapValue(exact.value);
  }
  return undefined;
}

export function findMetadataText(source: unknown, candidates: string[]): string | undefined {
  return firstText(findMetadataValue(source, candidates));
}

export function findMetadataNumber(source: unknown, candidates: string[]): number | undefined {
...
export function hasMeaningfulMetadata(source: unknown): boolean {
