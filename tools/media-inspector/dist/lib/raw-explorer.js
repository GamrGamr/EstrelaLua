...
function formatRawValue(value, language) {
    if (value instanceof Date && !Number.isNaN(value.getTime()))
        return new Intl.DateTimeFormat(language === "pt" ? "pt-PT" : "en-GB", { dateStyle: "medium", timeStyle: "medium" }).format(value);
...
function renderLeaf(key, value, t, language) {
...
function renderNode(key, value, t, language, depth = 0) {
    if (value === null || value instanceof Date || ArrayBuffer.isView(value) || value instanceof ArrayBuffer || typeof value !== "object")
        return renderLeaf(key, value, t, language);
    ...
    if (!entries.length)
        children.append(renderLeaf("value", Array.isArray(value) ? "[]" : "{}", t, language));
    for (const [childKey, childValue] of entries)
        children.append(renderNode(childKey, childValue, t, language, depth + 1));
    details.append(summary, actions, children);
    return details;
}
export function renderRawExplorer({ container, rawSources, query, source, language, t }) {
    container.replaceChildren();
    const selected = source === "all" ? rawSources : { [source]: rawSources[source] };
    ...
    for (const [key, value] of entries)
        container.append(renderNode(key, value, t, language));
}
