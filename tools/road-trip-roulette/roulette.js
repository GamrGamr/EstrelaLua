...
import { buildDestinationGuide, sourceLanguage, theForkSearchUrl } from "./guide-data.js?v=4";
...
function guideItemCard(item) {
  ...
  const isFood = item.category === "eat";
  const providerLabel = isFood ? t("searchInTheFork") : item.kind === "place" ? t("openInMaps") : t("searchInMaps");
  const providerUrl = isFood ? theForkSearchUrl() : mapsSearchUrl(item.query, item.point);
  return `<article class="guide-item ${item.kind === "search" ? "is-discovery" : "is-place"}">
    <strong>${label}</strong>
    <div class="guide-item-links">
      <a class="guide-map-link" href="${providerUrl}" target="_blank" rel="noopener noreferrer">${providerLabel} <span aria-hidden="true">↗</span></a>
...
function renderGuide(destination) {
  ...
  const depthHint = (items) => {
    const visits = items.filter((item) => item.category === "visit").length;
    if (language === "pt") return `${visits} ${visits === 1 ? "visita" : "visitas"} · ${items.length} ${items.length === 1 ? "ideia" : "ideias"}`;
    return `${visits} ${visits === 1 ? "visit" : "visits"} · ${items.length} ${items.length === 1 ? "idea" : "ideas"}`;
  };
  ...
  $$('[data-guide-depth]').forEach((button) => {
    button.setAttribute("aria-pressed", String(button.dataset.guideDepth === guideDepth));
    $("span", button).textContent = depthHint(guide[button.dataset.guideDepth]);
  });
  const planItems = guide[guideDepth];
  const categories = ["visit", "do", "eat", "practical"];
  $("#guide-content").innerHTML = `<div class="guide-categories">${categories.map((category) => {
    const items = planItems.filter((item) => item.category === category);
    if (!items.length) return "";
    const countLabel = language === "pt" ? `${items.length} ${items.length === 1 ? "sugestão" : "sugestões"}` : `${items.length} ${items.length === 1 ? "suggestion" : "suggestions"}`;
    return `<details class="guide-category"><summary><span><b>${t(category)}</b><small>${countLabel}</small></span><i aria-hidden="true">⌄</i></summary><div class="guide-category-content"><div class="guide-grid">${items.map(guideItemCard).join("")}</div></div></details>`;
  }).join("")}</div>`;
}
