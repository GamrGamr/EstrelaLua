...
test("Every destination has Mini, Medium, and Full guide depths", () => assert(destinations.every((destination) => { const guide = buildDestinationGuide(destination, stopMapQueries[destination.id], stopMapPoints[destination.id]); return guide.mini.length === 3 && guide.medium.length === 6 && guide.full.length === 8; })));
